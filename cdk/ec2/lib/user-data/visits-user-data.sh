...
tmux send-keys -t visits "export OTEL_SERVICE_NAME=$service_name" C-m
tmux send-keys -t visits "export OTEL_RESOURCE_ATTRIBUTES=service.namespace=pet-clinic" C-m
tmux send-keys -t visits "export OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:4318" C-m
tmux send-keys -t visits "export OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf" C-m
tmux send-keys -t visits "export OTEL_METRICS_EXPORTER=otlp" C-m
tmux send-keys -t visits "export OTEL_LOGS_EXPORTER=otlp" C-m
tmux send-keys -t visits "java -javaagent:/home/ec2-user/application-signals-demo/opentelemetry-javaagent.jar -jar spring-petclinic-visit*.jar" C-m
EOF